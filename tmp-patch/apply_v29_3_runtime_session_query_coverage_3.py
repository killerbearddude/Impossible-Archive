...
def expand_shared_query_predicate(segment: str) -> str:
    if "query == \"list-candidate-artifact-proposal-audits\"" in segment:
        return segment
...
def expand_shared_formatter(segment: str) -> str:
    if "format_candidate_artifact_proposal_audit_list(state, options.access)" in segment:
        return segment
    return insert_before_once(
        segment,
...
def expand_smoke(text: str) -> str:
    if "runtime_session_candidate_proposal_audit_list_and_detail" in text:
        return text
    insertion = (
        "run_session_and_grep runtime_session_candidate_proposal_audit_list_and_detail "
        "\"RuntimeSession initialized|CandidateArtifactProposalAudits visible to curator|CandidateArtifactProposalAudit:|- found: true|RuntimeSession ended\" "
        "\"--access curator --query list-candidate-artifact-proposal-audits\n"
...
print("Applied v29.3 RuntimeSession query coverage 3 patch.")
